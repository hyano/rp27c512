...
|gpio|-|GPIOの状態と設定を表示する。|e/c|
|gpio|"in"\|"out" pin|GPIOを入力か出力を設定する。(ext0-2のみに有効)|e/c|
|gpio|"pullup"\|"pulldown"\|"pullno" pin|GPIOをプルアップ/プルダウン/どちらもなしを設定する。|e/c|
|gpio|"set"\|"clr" pin|GPIOの値を設定する。(ext0-2のみに有効)|e/c|
|gpio|"pulse" pin [width]|GPIOの値を一定期間反転する。width[us]でパルス幅を指定する(defaultは100us)。(ext0-2のみに有効)|e/c|
|gpio|"save"|FLASH ROMにGPIOの設定を保存する。|e/c|
...
|e|[addr [value]]|デバイス上のデータを編集する。アドレスを省略すると前回の続きを編集する。値を省略すると対話型で入力になる。|e/c|
|m|start end dest|デバイス上のデータを移動する。|e/c|
|f|start end value|デバイス上の指定範囲を指定した値で埋める。|e/c|
...
|wlist|[start [end]]|capコマンドでキャプチャする範囲を表示する。start、endで表示する範囲を指定できる。|e/-|
